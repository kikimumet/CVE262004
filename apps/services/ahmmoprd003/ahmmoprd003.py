...
KAFKA_TOPIC         = _cfg.get("kafka.topic", "")
...
def get_next_hour_epoch() -> float:
    now = datetime.now()
    next_hour = (now + timedelta(hours=1)).replace(minute=0, second=0, microsecond=0)
    return next_hour.timestamp()

# ORACLE LOOKUP
def load_filter_from_oracle() -> Optional[Tuple[str, Set[str], str]]:
    conn = createOracleConnection(fernetKey=FERNET_KEY)
    if conn is None:
        logger.error(f"[{_SERVICE_NAME}] Koneksi Oracle gagal.")
        return None
    try:
        cursor = conn.cursor()
        cursor.execute("""
            SELECT DISTINCT B.VPARAMID, A.VINSNAME, A.VPRMRSLT, A.VURLAPI, B.VWCTID, B.VTECHNUM
        ...
        if not rows:
            logger.error(f"[{_SERVICE_NAME}] Oracle tidak mengembalikan data.")
            return None
        
        filter_wct = rows[0][4]
        api_url    = rows[0][3]
        all_params = {row[0] for row in rows if row[0]}

        technum_param_map = {}
        for row in rows:
            technum  = row[5] or "UNKNOWN"
            param_id = row[0]
            if technum not in technum_param_map:
                technum_param_map[technum] = set()
            if param_id:
                technum_param_map[technum].add(param_id)

        if not api_url:
            logger.error(f"[{_SERVICE_NAME}] VURLAPI kosong di Oracle.")
            return None

        logger.info(f"[{_SERVICE_NAME}] Oracle lookup OK → wct={filter_wct}, api_url={api_url}")
        for technum, params in technum_param_map.items():
            logger.info(f"[{_SERVICE_NAME}]   technum={technum}")
        logger.info(f"[{_SERVICE_NAME}]   all_params (union) = {all_params}")

        return filter_wct, all_params, api_url
...
def is_valid_message(val: dict, filter_wct: str) -> bool:
    return val.get("vwctid") == filter_wct
...
def run_pipeline(batch_data: list, filter_params: Set[str], api_url: str):
    ...
    technum_counter = Counter(row.get("technum") for row in valid_batch)
    param_counter   = Counter(row.get("param")   for row in valid_batch)
    logger.info(f"[{_SERVICE_NAME}] Breakdown technum lolos filter: {dict(technum_counter)}")
    logger.info(f"[{_SERVICE_NAME}] Breakdown param lolos filter  : {dict(param_counter)}")

    logger.info(f"[{_SERVICE_NAME}] Mengirim {len(valid_batch)} baris ke API...")
    ...
    if len(result_list) == 0:
        logger.warning(f"[{_SERVICE_NAME}] API result data kosong. Batch dilewati.")
        return
...
def start_stream():
    ...
    if filter_cfg is None:
        ...
        consumer.close()
        return
    filter_wct, filter_params, api_url = filter_cfg

    logger.info(f"[{_SERVICE_NAME}] Stream berjalan...")
    ...
    logger.info(f"[{_SERVICE_NAME}] API URL        : {api_url}")
    logger.info(f"[{_SERVICE_NAME}] Filter wct     : {filter_wct}")
    logger.info(f"[{_SERVICE_NAME}] Filter params  : {filter_params}\n")

    batch_buffer = []

    try:
        while True:
            next_ts = get_next_hour_epoch()
            next_dt = datetime.fromtimestamp(next_ts)
            logger.info(f"[{_SERVICE_NAME}] Trigger berikutnya: {next_dt.strftime('%Y-%m-%d %H:%M:%S')}")

            while datetime.now().timestamp() < next_ts:
                ...
                for tp, msgs in messages.items():
                    for msg in msgs:
                        ...
                        if isinstance(val, list):
                            batch_buffer.extend([
                                v for v in val
                                if isinstance(v, dict) and is_valid_message(v, filter_wct)
                            ])
                        elif isinstance(val, dict):
                            if is_valid_message(val, filter_wct):
                            ...
                            try:
                                ...
                                if isinstance(parsed, list):
                                    ...
                                    if is_valid_message(parsed, filter_wct):
                                        batch_buffer.append(parsed)
                            except json.JSONDecodeError:
                                continue

            now_str = datetime.now().strftime("%H:%M:%S")
            logger.info(f"[{_SERVICE_NAME}] [{now_str}] Trigger 1 Jam! Memproses batch...")
            ...
            filter_wct, filter_params, api_url = filter_cfg
            logger.info(
                f"[{_SERVICE_NAME}] Filter diperbarui dari Oracle → "
                f"wct={filter_wct}, params={filter_params}, api_url={api_url}"
            )

            if batch_buffer:
                logger.info(
                    f"[{_SERVICE_NAME}] [{now_str}] Memproses {len(batch_buffer)} baris... "
                    f"(leftover: {len(leftover_buffer)} baris)"
                )
                ...
                if leftover_buffer:
                    logger.info(
                        f"[{_SERVICE_NAME}] [{now_str}] Tidak ada data baru, "
                        f"retry {len(leftover_buffer)} baris leftover..."
                    )
                    run_pipeline([], filter_params, api_url)
                else:
                    logger.info(f"[{_SERVICE_NAME}] [{now_str}] Tidak ada data masuk selama 1 jam terakhir.")
