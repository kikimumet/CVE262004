...
def load_filter_from_oracle() -> Optional[Tuple[str, Set[str], str]]:
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE A.VWCTID = 'P9PSA0' AND A.VPRMRSLT = 'CYCLE-TIME-PREDICTION'
        """)
        ...
        logger.info(f"[{_SERVICE_NAME}] Oracle lookup OK → wct={filter_wct}, params={params}, api_url={api_url}")
        return filter_wct, params, api_url
...
def start_stream():
    ...
    logger.info(f"[{_SERVICE_NAME}] API URL        : {api_url}")
    logger.info(f"[{_SERVICE_NAME}] Filter aktif   : vwctid={filter_wct}")
    ...
    try:
        while True:
            ...
            # Poll Kafka sambil menunggu jam bulat
                ...
                for tp, msgs in messages.items():
                    for msg in msgs:
                        ...
                        if isinstance(val, list):
                            batch_buffer.extend([v for v in val if isinstance(v, dict) and is_valid_message(v, filter_wct)])
                            ...
                            try:
                                ...
                                if isinstance(parsed, list):
                                    batch_buffer.extend([v for v in parsed if isinstance(v, dict) and is_valid_message(v, filter_wct)])
            ...
            if filter_cfg is None:
                ...
                break
            filter_wct, filter_params, api_url = filter_cfg
            logger.info(f"[{_SERVICE_NAME}] Filter diperbarui dari Oracle → wct={filter_wct}, params={filter_params}, api_url={api_url}")

            if batch_buffer:
                logger.info(f"[{_SERVICE_NAME}] [{now_str}] Memproses {len(batch_buffer)} baris... (leftover: {len(leftover_buffer)} baris)")
                run_pipeline(batch_buffer, filter_params, api_url)
                batch_buffer = []
            else:
                if leftover_buffer:
                    logger.info(f"[{_SERVICE_NAME}] [{now_str}] Tidak ada data baru, retry {len(leftover_buffer)} baris leftover...")
