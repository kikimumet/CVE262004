...
# CONFIG
KAFKA_GROUP_ID         = "DEV_AHMOPRD001_PO_CASE1"
BATCH_INTERVAL_SECONDS = 3600   # 1 jam
RAPIDMINER_TIMEOUT     = 60
SAVE_DIR_RAW           = "data/P9PIA0/polymer_case1/raw"
...
ORACLE_INSIGHT_NAME    = "P9PIA0_IMM04_INSIGHT"
...
logger = logging.getLogger("case1_po")
...
def load_filter_from_oracle() -> Optional[Tuple[str, str, Set[str], str]]:
    ...
    if conn is None:
        logger.error("[case1_po] Koneksi Oracle gagal. Stream dihentikan.")
        return None
    ...
    try:
        ...
        if not rows:
            logger.error(f"[case1_po] Oracle tidak mengembalikan data untuk VINSNAME='{ORACLE_INSIGHT_NAME}'. Stream dihentikan.")
            return None
        ...
        if not api_url:
            logger.error("[case1_po] VURLAPI kosong di Oracle. Stream dihentikan.")
            return None

        logger.info(f"[case1_po] Oracle lookup OK → wct={wct}, technum={technum}, params={params}, api_url={api_url}")
        return wct, technum, params, api_url

    except Exception as e:
        logger.error(f"[case1_po] Gagal query Oracle: {e}. Stream dihentikan.")
        return None
    finally:
        conn.close()
...
def write_to_influx(result_list: list) -> bool:
    if not FERNET_KEY:
        logger.error("[case1_po] FERNET_KEY tidak ditemukan di environment variable. Skip write InfluxDB.")
        return False
    ...
    if client is None:
        logger.error("[case1_po] Koneksi InfluxDB gagal. Skip write.")
        return False
    ...
    try:
        ...
        for row in result_list:
            try:
                ...
                logger.warning(f"[case1_po] Skip row, timestamp tidak valid: {e}")
                continue
            ...
            for key, value in row.items():
                ...
                if is_nan_value(value):
                    logger.debug(f"[case1_po] Skip key '{key}' karena value NaN.")
                    nan_skipped += 1
                    continue
                ...
                try:
                    ...
                    logger.warning(f"[case1_po] Skip key '{key}', value bukan numerik: {value}")
                    continue

                if math.isnan(nvalue) or math.isinf(nvalue):
                    logger.debug(f"[case1_po] Skip key '{key}' karena nvalue={nvalue} tidak valid.")
                    nan_skipped += 1
                    continue
        ...
        if nan_skipped > 0:
            logger.info(f"[case1_po] Total key di-skip karena NaN: {nan_skipped}")

        if unknown_keys:
            logger.warning(f"[case1_po] Skip key format tidak dikenali: {unknown_keys}")

        if not points:
            logger.warning("[case1_po] Tidak ada point valid untuk ditulis ke InfluxDB.")
            return False

        client.write_points(points, time_precision="ms")
        logger.info(f"[case1_po] InfluxDB: {len(points)} point berhasil ditulis ke '{INFLUX_MEASUREMENT}'.")
        return True

    except Exception as e:
        logger.error(f"[case1_po] Gagal write ke InfluxDB: {e}")
        return False
    finally:
        client.close()
...
def run_pipeline(batch_data: list, filter_params: set, api_url: str):
    ...
    if batch_data:
        ...
        if not filtered:
            logger.warning("[case1_po] Tidak ada data valid setelah filter kolom.")
            return
        ...
        if not isinstance(json_str, str):
            logger.error("[case1_po] Gagal rename parameter. Batch dilewati.")
            return
        ...
        if not isinstance(json_str, str):
            logger.error("[case1_po] Gagal convert time. Batch dilewati.")
            return
    ...
    if leftover_buffer:
        logger.info(f"[case1_po] Menggabungkan {len(leftover_buffer)} baris leftover dari batch sebelumnya...")
        valid_batch = leftover_buffer + valid_batch
        leftover_buffer = []
        logger.info(f"[case1_po] Total setelah gabung leftover: {len(valid_batch)} baris.")

    if not valid_batch:
        logger.warning("[case1_po] Tidak ada data valid setelah proses awal.")
        return

    # 3.6 SAVE RAW DATA KAFKA
    logger.info(f"[case1_po] Menyimpan {len(valid_batch)} baris data mentah Kafka...")
    raw_save_status = saveAndAppend(json.dumps(valid_batch), base_dir=SAVE_DIR_RAW)
    if not raw_save_status:
        logger.warning("[case1_po] Gagal menyimpan data mentah Kafka.")
    else:
        logger.info(f"[case1_po] Data mentah Kafka tersimpan di: {SAVE_DIR_RAW}")

    # 4. FILTER PARAM
    param_counts = Counter(row.get("param") for row in valid_batch)
    logger.info(f"[case1_po] Distribusi param sebelum filter: {dict(param_counts)}")
    ...
    if not valid_batch_filtered:
        logger.warning(
            f"[case1_po] Tidak ada param yang memenuhi syarat (harus dalam {filter_params}). "
        )
    ...
    logger.info(f"[case1_po] Param lolos filter: {dict(param_lolos)}")

    wct     = valid_batch[0].get("wct", "UNKNOWN")
    technum = valid_batch[0].get("technum", "UNKNOWN")

    logger.info(f"[case1_po] Mengirim {len(valid_batch)} baris ke API... [wct={wct}, technum={technum}]")
    logger.info(f"[case1_po] API URL: {api_url}")
    ...
    if result_list is None:
        logger.error("[case1_po] API gagal. Menyimpan batch ke leftover untuk retry batch berikutnya.")
        leftover_buffer = valid_batch
        return

    if len(result_list) == 0:
        logger.warning("[case1_po] API result data kosong. Batch dilewati.")
        return

    logger.info(f"[case1_po] API selesai. {len(valid_batch)} → {len(result_list)} baris.")
...
def start_stream():
    if not FERNET_KEY:
        logger.error("[case1_po] FERNET_KEY tidak ditemukan di .env. Stream dihentikan.")
        return
    ...
    try:
        ...
        logger.error(f"[case1_po] Gagal koneksi ke Kafka: {e}")
        return

    # INITIAL ORACLE LOOKUP
    filter_cfg = load_filter_from_oracle()
    if filter_cfg is None:
        logger.error("[case1_po] Oracle lookup awal gagal. Stream dihentikan.")
        consumer.close()
        return
    filter_wct, filter_technum, filter_params, api_url = filter_cfg

    logger.info(f"[case1_po] Stream berjalan... Batch per {BATCH_INTERVAL_SECONDS} detik")
    logger.info(f"[case1_po] Topic       : {KAFKA_TOPIC}")
    logger.info(f"[case1_po] Group ID    : {KAFKA_GROUP_ID}")
    logger.info(f"[case1_po] API URL     : {api_url}")
    logger.info(f"[case1_po] Filter aktif: vwctid={filter_wct}, vmachineid={filter_technum}")
    logger.info(f"[case1_po] Filter param: {filter_params}\n")
    ...
    try:
        while True:
            ...
            if elapsed >= BATCH_INTERVAL_SECONDS:
                ...
                filter_cfg = load_filter_from_oracle()
                if filter_cfg is None:
                    logger.error("[case1_po] Oracle lookup gagal saat refresh. Stream dihentikan.")
                    break
                filter_wct, filter_technum, filter_params, api_url = filter_cfg
                logger.info(f"[case1_po] Filter diperbarui dari Oracle → wct={filter_wct}, technum={filter_technum}, params={filter_params}, api_url={api_url}")

                now = datetime.now().strftime("%H:%M:%S")
                if batch_buffer:
                    logger.info(f"[case1_po] [{now}] Memproses {len(batch_buffer)} baris... (leftover: {len(leftover_buffer)} baris)")
                    run_pipeline(batch_buffer, filter_params, api_url)
                    batch_buffer = []
                else:
                    if leftover_buffer:
                        logger.info(f"[case1_po] [{now}] Tidak ada data baru, retry {len(leftover_buffer)} baris leftover...")
                        run_pipeline([], filter_params, api_url)
                    else:
                        logger.info(f"[case1_po] [{now}] Tidak ada data dalam {BATCH_INTERVAL_SECONDS} detik terakhir.")

                start_time = time.time()

    except KeyboardInterrupt:
        logger.info("[case1_po] Stream dihentikan manual.")
    except Exception as e:
        logger.error(f"[case1_po] Fatal Error: {e}")
    finally:
        consumer.close()
