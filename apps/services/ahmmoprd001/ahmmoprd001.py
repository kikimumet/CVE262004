...
BATCH_INTERVAL_SECONDS = int(_cfg.get("batch_interval_seconds", 3600))
...
def start_stream():
    ...
    logger.info(f"[{_SERVICE_NAME}] Stream berjalan... Batch per {BATCH_INTERVAL_SECONDS} detik")
    ...
    try:
        while True:
            ...
            if time.time() - start_time >= BATCH_INTERVAL_SECONDS:
                ...
                if batch_buffer:
                    logger.info(f"[{_SERVICE_NAME}] [{now}] Memproses {len(batch_buffer)} baris... (leftover: {len(leftover_buffer)} baris)")
                    run_pipeline(batch_buffer, filter_params, api_url)
                    batch_buffer = []
                else:
                    if leftover_buffer:
                        logger.info(f"[{_SERVICE_NAME}] [{now}] Tidak ada data baru, retry {len(leftover_buffer)} baris leftover...")
                        run_pipeline([], filter_params, api_url)
                    else:
                        logger.info(f"[{_SERVICE_NAME}] [{now}] Tidak ada data dalam {BATCH_INTERVAL_SECONDS} detik terakhir.")
                start_time = time.time()
