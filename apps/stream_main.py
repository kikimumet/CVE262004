...
SAVE_DIR_RAW           = "apps/data/rawKafka"
SAVE_DIR_RESULT        = "apps/data/resultResponseAPI"
...
def write_to_influx(result_list: list) -> bool:
    ...
    try:
        ...
        for row in result_list:
            ...
            for key, value in row.items():
                ...
                if is_nan_value(value):
                    logger.debug(f"Skip key '{key}' karena value NaN.")
                    nan_skipped += 1
                    continue
                ...
                if math.isnan(nvalue) or math.isinf(nvalue):
                    logger.debug(f"Skip key '{key}' karena nvalue={nvalue} tidak valid.")
                    nan_skipped += 1
                    continue
        ...
        if nan_skipped > 0:
            logger.info(f"Total key di-skip karena NaN: {nan_skipped}")
